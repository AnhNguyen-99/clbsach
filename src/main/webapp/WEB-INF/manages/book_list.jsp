...
<body>
	...
	<div id="wrapper">
		...
		<div id="page-wrapper">
			<div class="container-fluid">
				<div class="row">
					<div class="col-lg-12">
						<h1 class="page-header">Danh sách book</h1>
					</div>
				</div>
				<div class="row">
					<div class="col-lg-12">
										...
										<tbody>
											<c:forEach items="${listBook}" var="item">
												<tr>
													...
													<td>${item.publishing.publishingName}</td>
													<td>${item.author.authorName}</td>
													<td>${item.quantity}</td>
													<td>${item.yearPublishing}</td>
													<td>${item.createDate}</td>
													<td><a class='btn btn-default'
														href='${pageContext.request.contextPath}/manages/book/update/${item.bookId}'><i
															class='fa fa-edit'></i></a>
														<p class='btn btn-default'
															onclick='checkdel(${item.bookId})'>
	...
	<script>
        function checkdel(bookId) {
            var bookId = bookId;
            var link = "${pageContext.request.contextPath}/manages/book/delete/" + bookId;
